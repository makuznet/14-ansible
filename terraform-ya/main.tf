...
resource "yandex_vpc_network" "yanet1" {
  name = "yanet1"
}

resource "yandex_vpc_subnet" "yasubnet1" {
  name           = "yasubnet1"
  zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.yanet1.id
...
resource "yandex_compute_instance" "ya" {
  ...
  network_interface {
    subnet_id  = yandex_vpc_subnet.yasubnet1.id
    nat        = true
...
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec" {
    command = "cd ..; ansible-playbook -i terraform-ya/inventory.yml playbooks/main.yml --ask-vault-password"
  }
